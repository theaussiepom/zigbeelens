...
Shared product language for ZigbeeLens decisions, the Mesh investigation
workspace, Device Story, incidents, reports, and companion surfaces.
...
1. Device summary and current decision
2. Why the device is or is not worth reviewing
3. Device Story and recent activity
4. Snapshot history
5. Data coverage and limitations
6. Evidence details and open incidents
7. What to check next — only if useful
8. Create device report
...
Human-facing priorities: **Review first**, **Worth reviewing**, **Lower priority**
...
A redacted exact `ReportDetailV3` report generated by Core from the subject the
user is already investigating. Device Detail creates a device report, Incident
Detail creates an incident report, Network Detail and Mesh / Investigate create
a network report, and Reports creates a full-estate report. Saved reports are
stored locally in SQLite and can be downloaded as JSON, YAML, or Markdown.

Human-facing: **Create device report**, **Create incident report**, **Create
network report**, **Create full report**, **Saved reports**

Practical qualifier when needed: “This is an evidence-backed snapshot, not
root-cause proof or a live routing map.”

Avoid: AI report, graph export, client-side evidence summary, diagnostic dump,
export graph data dump.
