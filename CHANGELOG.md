...
- **Phase 7A query bounds:** additive incident `order=recent` closes Overview PR #83 lifecycle-ranked recent-change omission; topology overview uses bulk latest-complete snapshots; device snapshot history is SQL-windowed with bulk links; metric windows use deterministic `id` tie-break; shared-availability instability reads offline transitions in SQL; migration `013` index-only query plans; Track 5 baselines frozen, Phase 7A tip published (execute/commit cardinalities unchanged)
