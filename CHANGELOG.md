...
- **Phase 7C2 evidence status:** Phase 7C2 is complete: PR #108 merged the one-source synthetic S1–S9 set captured from `af04ee906b71de77ee6e0eb5d866c0647d502410`, with manifest-backed provenance, privacy review, and green remote checks. Phase 7D remains blocked pending final HACS synchronization and artifact freeze.
...
- **Topology-history coverage:** Device Story, UI, and report coverage now count
  every selected complete capture as either an available or limited layout.
  Mixed history is visibly partial, limited captures remain explicit, and only
  available layouts support observed/not-observed device-presence conclusions.
